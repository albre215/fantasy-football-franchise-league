...
export async function POST(_request: Request, { params }: RouteContext) {
  try {
    ...
    const auction = await inauguralAuctionService.simulateStep(params.seasonId, actingUserId);
    ...
    return NextResponse.json({ error: "Unable to simulate the next step." }, { status: 500 });
  }
}
